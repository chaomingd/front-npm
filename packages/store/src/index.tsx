...
> {
  ...
  useGetState = <Key extends keyof TState>(
    keys?: Key[],
    equalityFn?: TEqualityFn<TState>,
    name?: string,
  ) => {
    const state = this.useSelector((prevState, nextState) => {
      if (keys && shallowEqualKeys(prevState, nextState, keys as string[])) {
        return true;
      }
  ...
  useUpdate = <Key extends keyof TState>(keys?: Key[], equalityFn?: TEqualityFn<TState>) => {
    this.useGetState(keys, equalityFn);
  };
  subscribeWithKeys<Key extends keyof TState>(
    ...
    return this.subscribe((_, silent) => {
      ...
      if (keys && shallowEqualKeys(this._preState, nextState, keys as string[])) {
        return;
      }
  ...
  useSubscribe = <Key extends keyof TState>(
    func: TSubscribeFunc<TState, TEffects>,
    options?: {
      keys?: Key[];
      equalityFn?: TEqualityFn<TState>;
